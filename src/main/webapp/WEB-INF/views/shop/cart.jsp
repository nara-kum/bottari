								...
								<main class="main">
									<div class="container">
										<div class="cart-toolbar">
											<button type="button" id="cart-bulk-delete" class="btn-bulk-delete">선택
												삭제</button>
										</div>
																...
																<div class="optionContainer" id="option-${vo.cart_no}">
																	...
																	<div class="row-flex-box">
																		<c:forEach items="${vo.options}" var="option">
																			<label
																				for="option-${option.option_no}-${vo.cart_no}">${option.option_name}</label>
																			<select
																				id="option-${option.option_no}-${vo.cart_no}"
																				class="option-select"
																				data-option-no="${option.option_no}"
																				data-cart-no="${vo.cart_no}">
																				<c:forEach items="${option.detailList}"
																					var="detail">
																					<c:choose>
																						<c:when
																							test="${detail.detailoption_no == vo.detailoption_no}">
																							<option
																								value="${detail.detailoption_no}"
																								selected>
																								${detail.detailoption_name}
																							</option>
																						</c:when>
																						<c:otherwise>
																							<option
										...
										<div class="summary ">
											<div class="text-16">
												상품금액: <span id="product-total">
													<fmt:formatNumber value="${total_price}" type="currency"
														currencySymbol="" />
												</span>원
											</div>
											<div class="text-16">
												배송비: <span id="shipping-cost">
													<fmt:formatNumber value="${shipping_cost}" type="currency"
														currencySymbol="" />
												</span>원
											</div>
											<div class="text-18 bold">
												총 결제금액: <span id="final-total">
													<fmt:formatNumber value="${total_price + shipping_cost}"
														type="currency" currencySymbol="" />
												</span>원
											</div>
										</div>
